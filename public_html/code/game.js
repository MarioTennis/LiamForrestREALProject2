...
function Player(pos) {
  this.pos = this.startPos = pos.plus(new Vector(0, -0.5));
...
function Portal(pos) {
	this.basePos = this.pos = pos.plus(new Vector(.25, .7));
...
Goal.prototype.act = function(step){
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.actors = this.actors.filter(function(other) {
	  ...
	  if (playerXSpeed != 20) {
		playerXSpeed = 20;
		setTimeout(function(){playerXSpeed = 7;}, 2000);
	  }
  } else if (type == "flag") {
